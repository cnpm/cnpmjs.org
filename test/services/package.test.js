...
describe('test/services/package.test.js', () => {
  ...
  describe('addModuleTag()', function() {
    it('should add latest tag to 1.0.0', function* () {
      let r = yield createModule('test-addModuleTag-module-name', '1.0.0');
      const tag = yield Package.addModuleTag(r.name, 'latest', r.version);
      should.exist(tag);
      tag.id.should.above(0);

      r = yield createModule('test-addModuleTag-module-name', '1.1.0');
      const tag2 = yield Package.addModuleTag(r.name, 'latest', r.version);
      should.exist(tag2);
      tag.id.should.equal(tag2.id);

      const tag3 = yield Package.getModuleTag(r.name, 'latest');
      tag3.id.should.equal(tag2.id);
    });

    it('should return null when module not exists', function* () {
      const tag = yield Package.addModuleTag('not-exists', 'latest', '1.0.0');
  ...
  describe('getModuleByTag()', function() {
    it('should get latest module', function* () {
      const r = yield createModule('test-getModuleByTag-module-name', '1.0.0');
      const tag = yield Package.addModuleTag(r.name, 'latest', r.version);
      should.exist(tag);

      const mod = yield Package.getModuleByTag(r.name, 'latest');
      should.exist(mod);
    ...
    it('should return null when tag not exists', function* () {
      const r = yield Package.getModuleByTag('some', 'not-exists');
      should.not.exist(r);
    });
  });
  ...
  describe('listPublicModuleNamesByUser(), listPublicModulesByUser()', function() {
    ...
    it('should got all public module names', function* () {
      const names = yield Package.listPublicModuleNamesByUser('listPublicModuleNamesByUser-user');
    ...
    it('should got all public modules', function* () {
      const mods = yield Package.listPublicModulesByUser('listPublicModuleNamesByUser-user');
    ...
    it('should return [] when user not exists', function* () {
      const mods = yield Package.listPublicModulesByUser('listPublicModuleNamesByUser-user-not-exists');
  ...
  describe('listModulesByName()', function() {
    it('should return [] when module name not exists', function* () {
      const mods = yield Package.listModulesByName('not-exists-module');
      mods.should.length(0);
    });
    ...
    it('should return all version modules', function* () {
      ...
      const modules = yield Package.listModulesByName('test-listModulesByName-module-1');
  ...
  describe('listPrivateModulesByScope()', function() {
    it('should return [] when scope not exists', function* () {
      const modules = yield Package.listPrivateModulesByScope('@not-exists');
      modules.should.eql([]);
    });
    ...
    it('should work', function* () {
      ...
      const modules = yield Package.listPrivateModulesByScope('@cnpm-test');
      modules.should.length(2);
      modules[0].name.should.containEql('@cnpm-test/test-listPrivateModules-module-');
    });
  });

  describe('listPublicModuleNamesSince(), listAllPublicModuleNames()', () => {
  ...
  describe('getModuleLastModified()', function() {
    it('should get a datetime', function* () {
      ...
      const t = yield Package.getModuleLastModified('test-getModuleLastModified-module-0');
      t.should.be.a.Date;
    });

    it('should get null when module not exists', function* () {
      const t = yield Package.getModuleLastModified('test-getModuleLastModified-module-not-exists');
      should.ok(t === null);
    });
  });
  ...
  describe('removeModulesByName()', function() {
    it('should remove all', function* () {
      ...
      let mods = yield Package.listModulesByName('test-removeModulesByName-module-1');
      mods.should.length(3);
  ...
  describe('removeModulesByNameAndVersions()', function() {
    it('should remove some versions', function* () {
      ...
      let mods = yield Package.listModulesByName('test-removeModulesByNameAndVersions-module-1');
  ...
  describe('removeModuleTags()', function() {
    it('should remove all tags by name', function* () {
      const r2 = yield createModule('test-removeModuleTagsByName2-module-name', '1.0.0');
      let tag = yield Package.addModuleTag(r2.name, 'latest', r2.version);
      should.exist(tag);

      const r = yield createModule('test-removeModuleTagsByName-module-name', '1.0.0');
      tag = yield Package.addModuleTag(r.name, 'latest', r.version);
      should.exist(tag);
      tag = yield Package.addModuleTag(r.name, 'beta', r.version);
      should.exist(tag);

      let tags = yield Package.listModuleTags(r.name);
      tags.should.length(2);
      yield Package.removeModuleTags(r.name);
      tags = yield Package.listModuleTags(r.name);
      tags.should.eql([]);

      const tags2 = yield Package.listModuleTags(r2.name);
  ...
  describe('removeModuleTagsByIds()', function() {
    it('should remove tags by ids', function* () {
      const r = yield createModule('test-removeModuleTagsByIds-module-name', '1.0.0');
      const tag1 = yield Package.addModuleTag(r.name, 'latest', r.version);
      should.exist(tag1);
      const tag2 = yield Package.addModuleTag(r.name, 'beta', r.version);
      should.exist(tag2);
      const tag3 = yield Package.addModuleTag(r.name, 'beta2', r.version);
      should.exist(tag3);
  ...
  describe('removeModuleTagsByNames()', function() {
    it('should remove some tags', function* () {
      const r = yield createModule('test-removeModuleTagsByNames-module-name', '1.0.0');
      const tag1 = yield Package.addModuleTag(r.name, 'latest', r.version);
      should.exist(tag1);
      const tag2 = yield Package.addModuleTag(r.name, 'beta', r.version);
      should.exist(tag2);
  ...
  describe('addModule()', function() {
    it('should success ad he@0.3.6', function* () {
      const sourcePackage = require('../fixtures/0.3.6.json');
      const mod = {
        version: sourcePackage.version,
        name: sourcePackage.name,
        package: sourcePackage,
      ...
      mod.package.dist = dist;
      const result = yield Package.saveModule(mod);
      result.id.should.be.a.Number;
      const item = yield Package.getModuleById(result.id);
      ...
      const r = yield Package.getModule(mod.name, mod.version);
  ...
  describe('getModuleByRange()', function() {
    it('should get undefined when not match semver range', function* () {
      ...
      const mod = yield Package.getModuleByRange('test-getModuleByRange-module-0', '~2.1.0');
    ...
    it('should get package with semver range', function* () {
      ...
      const mod = yield Package.getModuleByRange('test-getModuleByRange-module-1', '1');
      mod.package.name.should.equal(mod.name);
    ...
    it('should get package with semver range when have invalid version', function* () {
      ...
      const mod = yield Package.getModuleByRange('test-getModuleByRange-module-2', '1');
      mod.package.name.should.equal(mod.name);
  ...
  describe('updateModulePackage()', function() {
    it('should update not exists package return null', function* () {
      const r = yield Package.updateModulePackage(101010101, {});
      should.not.exist(r);
    });
    ...
    it('should update exists package', function* () {
      ...
      mod.package.dist = dist;
      yield Package.saveModule(mod);
      const result = yield Package.getModule(mod.name, mod.version);
      result.package.should.eql(sourcePackage);

      const newPackage = { foo: 'bar' };
  ...
  describe('updateModulePackageFields()', function() {
    it('should return null when update not exists module', function* () {
      const r = yield Package.updateModulePackageFields(123123123, { foo: 'bar' });
      should.not.exist(r);
    });

    it('should return updated module instance', function* () {
      const r = yield createModule('test-updateModulePackageFields-name', '1.0.0');
      should.exist(r);
      const r1 = yield Package.updateModulePackageFields(r.id, { foo: 'update for field' });
      r1.id.should.equal(r.id);
      const r2 = yield Package.getModuleById(r1.id);
  ...
  describe('updateModuleReadme()', function() {
    it('should return null when update not exists module', function* () {
      const r = yield Package.updateModuleReadme(123123123, 'test updateModuleReadme');
      should.not.exist(r);
    });

    it('should return updated module instance', function* () {
      const r = yield createModule('test-updateModuleReadme-name', '1.0.0');
      should.exist(r);
      const r1 = yield Package.updateModuleReadme(r.id, 'test updateModuleReadme');
      r1.id.should.equal(r.id);
      const r2 = yield Package.getModuleById(r1.id);
  ...
  describe('updateModuleDescription()', function() {
    it('should return null when update not exists module', function* () {
      const r = yield Package.updateModuleDescription(123123123, 'test updateModuleDescription');
      should.not.exist(r);
    });

    it('should return updated module instance', function* () {
      const r = yield createModule('test-updateModuleDescription-name', '1.0.0');
      should.exist(r);
      const r1 = yield Package.updateModuleDescription(r.id, 'test updateModuleDescription');
      r1.id.should.equal(r.id);
      const r2 = yield Package.getModuleById(r1.id);
  ...
  describe('updateModuleLastModified()', function() {
    it('should return null when module not exists', function* () {
      const r = yield Package.updateModuleLastModified('not-exists-module-name');
      should.not.exist(r);
    });

    it('should return the update module when update lastTime exists', function* () {
      const r1 = yield createModule('test-update-module-last-modified-package-name', '1.0.0');
      yield sleep(1100);
      yield Package.updateModuleLastModified(r1.name);
      const r2 = yield Package.getModule(r1.name, r1.version);
  ...
  describe('addKeyword()', function() {
    it('should add duplicat keywords', function* () {
      const r = yield Package.addKeyword({
      ...
      const r2 = yield Package.addKeyword({
        name: 'addKeyword-test-name',
        keyword: 'addKeyword-test-keyword',
        description: 'addKeyword-test-description',
      });
      ...
      const r3 = yield Package.addKeyword({
        name: 'addKeyword-test-name',
        keyword: 'addKeyword-test-keyword',
  ...
  describe('search()', function() {
    ...
    it('should search modules', function* () {
      const data = yield Package.search('test');
    ...
    it('should search match keywords modules', function* () {
      const data = yield Package.search('aa');
    ...
    it('should search return empty', function* () {
      const data = yield Package.search('emptyemptyemptyempty');
      data.should.eql({
        keywordMatchs: [],
        searchMatchs: [],
      });
  ...
  describe('addDependencies(), listDependencies(), listDependents()', function() {
    it('should add some module dependencies', function* () {
      let rows = yield Package.addDependencies('addDependencies-test-module', [
      ...
      const dependents = yield Package.listDependents('addDependencies-dep1');
      dependents.should.eql([
        'addDependencies-test-module',
      ]);

      let names = yield Package.listDependents('addDependencies-dep1');
  ...
  describe('addStar(), removeStar(), listStarUserNames(), listUserStarModuleNames()', function() {
    it('should star a module and remove that star', function* () {
      let row = yield Package.addStar('addStar-module', 'addStar-user');
      row.id.should.be.a.Number;
      row = yield Package.addStar('addStar-module', 'addStar-user');
      row.id.should.be.a.Number;

      let users = yield Package.listStarUserNames('addStar-module');
      users.should.eql([ 'addStar-user' ]);
      let names = yield Package.listUserStarModuleNames('addStar-user');
