...
describe('controllers/registry/package/update.test.js', function() {
  ...
  before(function(done) {
    const pkg = utils.getPackage('@cnpmtest/testmodule-update-1', '1.0.0', utils.otherUser);
    request(app.listen())
    .put('/' + pkg.name)
    ...
    .expect(201, function(err) {
      ...
      const pkg = utils.getPackage('@cnpmtest/testmodule-update-1', '2.0.0', utils.otherUser);
      request(app.listen())
      .put('/' + pkg.name)
      .set('authorization', utils.otherUserAuth)
      .send(pkg)
  ...
  describe('PUT /:name/-rev/:rev updatePrivateModuleMaintainers()', function() {
    ...
    describe('forcePublishWithScope = true', function() {
      before(function(done) {
        let pkg = utils.getPackage('@cnpm/testmodule-update-1', '0.0.1', utils.otherUser);
        request(app)
        .put('/' + pkg.name)
        .set('authorization', utils.otherUserAuth)
        .send(pkg)
