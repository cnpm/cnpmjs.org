...
describe('test/controllers/registry/package/save.test.js', function() {
  ...
  describe('no @scoped package', function() {
    ...
    before(function(done) {
      ...
      const pkg = utils.getPackage('testmodule-new-1', '0.0.1', utils.admin);
      ...
      .expect(201, function(err) {
        ...
        const pkg = utils.getPackage('testmodule-new-2', '1.0.0', utils.admin);
    ...
    it('should save dependents', function* () {
      let names = yield packageService.listDependents('bytetest-1');
    ...
    it('should auto add latest tag when it not exists', function(done) {
      const pkg = utils.getPackage('testmodule-no-latest', '0.0.1', utils.admin);
    ...
    it('should publish with tag', function(done) {
      const pkg = utils.getPackage('testmodule-no-latest', '0.0.2', utils.admin);
      const tags = pkg['dist-tags'];
      tags.v2 = tags.latest;
      delete tags.latest;
      request(app.listen())
    ...
    it('should 403 when attachments length wrong', function(done) {
      const pkg = utils.getPackage('testmodule-new-1', '0.0.3', utils.admin);
    ...
    it('should 403 when user is not maintainer', function(done) {
      const pkg = utils.getPackage('testmodule-new-1', '0.0.2', utils.otherAdmin2);
      request(app.listen())
      .put('/' + pkg.name)
  ...
  describe('@scoped package', function() {
    before(function(done) {
      const pkg = utils.getPackage('@cnpmtest/testmodule-new-1', '0.0.1', utils.admin);
      pkg.versions['0.0.1'].dependencies = {
        bytetest2: '~0.0.1',
        mocha: '~1.0.0',
      };
      ...
      .expect(201, function(err) {
        should.not.exist(err);
        const pkg = utils.getPackage('@cnpmtest/testmodule-new-2', '1.0.0', utils.admin);
        pkg.versions['1.0.0'].dependencies = {
          bytetest2: '~0.0.1',
          mocha: '~1.0.0',
        };
    ...
    it('should save dependents', function* () {
      let names = yield packageService.listDependents('bytetest2');
      names.should.length(2);
